...
TiltChrome.Visualization = function(canvas, controller, ui) {
  ...
  /**
   * Importance classification array.
   */
  importance_array = new Array(),
  ...
  /**
   * Labels and word lists making up the importance classification array.
   */
  importance_array["not important"] =
    "movie concert breakfast lunch dinner snack";

  importance_array["slightly important"] =
    "grocery laundry shopping oil change tire rotation brakes seminar talk";

  importance_array["important"] =
    "birthday meet meeting research homework job teach";

  importance_array["very important"] =
    "qualifying preliminary final exam court wedding funeral graduation";
  ...
  var setupVisualization = function() {
    ...
    for (var label in importance_array) {
      teachAs(label, importance_array[label]);
    }
    ...
    Tilt.Document.traverse(function(node, depth, index, uid,
      ...
      if (coordWidth > 2 && coordHeight > 2) {
        ...
        // thickness for our important elements
        ...
        // VERY HACKY!!!  Works only for Google Calendar
        if (window.content.location.href.indexOf("google.com/calendar") != -1) {

          // Google Calendar
          if (className === "cbrd") {

            // Detected a calendar event cell at this point

            // Process the text and classify it
            var event = node.childNodes[1].childNodes[0].innerHTML;
            var probs = guess(event);

            // Find the classification with the highest probability
            // Everything seems to be sorted by probability
            var rank = 0;
            if (probs[0].probability >= 0.05) { // reduce # of false alarms

              // classify the event
              for (var label in importance_array) {
                if (probs[0].classification === label) {
                  break;
                }
                rank++;
              }

              // safeguard just in case a serious bug exists in the Bayes
              // classifier
              if (rank == importance_array.length) {
                rank = 0;
              }
            }

            // Use it
            // Depth (or thickness) is directly proportional to level of
            // importance
            var dz = depth_factor * rank * thickness;
            real_thickness += dz;
            z += dz;
          }
        }
        else {
          // TODO Any other website besides Google Calendar
          // TODO analyze arbitrary website for text
        }
        ...
        // texture coordinates which are not shared by the other faces
                        x,     y + h, z - real_thickness,        /* top */    // 4
                        x + w, y + h, z - real_thickness,                     // 5
                        x + w, y + h, z,                                 // 6
                        x,     y + h, z,                                 // 7
                        x,     y,     z,                    /* bottom */ // 8
                        x + w, y,     z,                                 // 9
                        x + w, y,     z - real_thickness,                     // 10
                        x,     y,     z - real_thickness);                    // 11
